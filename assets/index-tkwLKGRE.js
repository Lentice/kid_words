...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "app-footer", children: "Made with ❤️ for kids" })
  ] });
}
function useWordData() {
  ...
  reactExports.useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const base = "/kid_words/";
        const [w2, s, p2] = await Promise.all([
          fetch(`${base}data/words.json`).then((r2) => r2.json()),
          fetch(`${base}data/sections.json`).then((r2) => r2.json()),
          fetch(`${base}data/parts_of_speech.json`).then((r2) => r2.json())
        ]);
        if (cancelled) return;
        w2.sort((a, b) => a.id - b.id);
        s.sort((a, b) => a.id - b.id);
        p2.sort((a, b) => a.id - b.id);
        setWords(w2);
        setSections(s);
        setParts(p2);
      } catch (e) {
...
function Flashcard({ item, section, pos, learned, onPrev, onNext, onToggleLearned }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn accent", onClick: speakExample, children: "發音例句" }),
...
function SectionPicker({ sections, selectedIds, onChange }) {
  const toggle = (id2) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel", children: [
    ...
    open && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-list", style: { marginTop: 8 }, children: sections.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "section-item", children: [
...
function saveProgress({ learnedIds, lastIndex, selectedSectionIds, range }) {
  const data = read();
...
function Learn() {
  ...
  const [index2, setIndex] = reactExports.useState(saved.lastIndex || 0);
  ...
  reactExports.useEffect(() => {
    if (index2 >= filtered.length) setIndex(0);
  }, [filtered.length]);
  reactExports.useEffect(() => {
    saveProgress({ selectedSectionIds: selected });
  }, [selected]);
  const current = filtered[index2] || null;
  const pos = `${index2 + 1} / ${filtered.length}`;
  reactExports.useEffect(() => {
    setDwellReady(false);
    if (!current) return;
  ...
  if (loading) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: "載入中…" });
  if (error) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: "載入資料時發生錯誤" });
  if (!filtered.length) return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stack", style: { maxWidth: 900, width: "100%" }, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(SectionPicker, { sections, selectedIds: selected, onChange: setSelected }),
...
function Quiz() {
  ...
  const rawPool = reactExports.useMemo(() => bySections(selected), [bySections, selected, words]);
...
function Admin() {
  ...
  const [selected, setSelected] = reactExports.useState([]);
  const filtered = reactExports.useMemo(() => bySections(selected), [bySections, selected, words]);
  const clearAll = () => {
